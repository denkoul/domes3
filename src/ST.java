...
class ST {
    ...
    private String to_print = ""; //A string to store all the data to print on the PrintTree


    //The two constructors of the search tree (with comparator)
    public ST() {
    ...
    void insertWarehouse(int nodeid, String name) {
        ...
        if (find(nodeid) != null) {
            System.err.println("Warehouse with id: " + nodeid + " already exists and can not be inserted again!");
            return;
        }
    ...
    private TreeNode removeR(TreeNode h, int id) {
        ...
        int w = h.id;
        if (id<w) h.l = removeR(h.l,id);
        if (w<id) h.r = removeR(h.r,id);
